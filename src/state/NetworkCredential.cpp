...
bool WifiCredentialStore::saveToFile() const {
  ...
  for (const auto& cred : credentials) {
    ...
    INX_SERIAL.printf("[%lu] [WCS] Saving SSID: %s, password length: %zu\n", millis(), cred.ssid.c_str(),
                  cred.password.size());
  ...
  file.close();
  INX_SERIAL.printf("[%lu] [WCS] Saved %zu WiFi credentials to file\n", millis(), credentials.size());
  return true;
}
...
bool WifiCredentialStore::loadFromFile() {
  ...
  if (version != WIFI_FILE_VERSION) {
    INX_SERIAL.printf("[%lu] [WCS] Unknown file version: %u\n", millis(), version);
    file.close();
    return false;
  }
  ...
  for (uint8_t i = 0; i < count && i < MAX_NETWORKS; i++) {
    ...
    INX_SERIAL.printf("[%lu] [WCS] Loaded SSID: %s, obfuscated password length: %zu\n", millis(), cred.ssid.c_str(),
                  cred.password.size());
    obfuscate(cred.password);
    INX_SERIAL.printf("[%lu] [WCS] After deobfuscation, password length: %zu\n", millis(), cred.password.size());

    credentials.push_back(cred);
  }

  file.close();
  INX_SERIAL.printf("[%lu] [WCS] Loaded %zu WiFi credentials from file\n", millis(), credentials.size());
  return true;
}
...
bool WifiCredentialStore::addCredential(const std::string& ssid, const std::string& password) {
  ...
  if (cred != credentials.end()) {
    ...
    INX_SERIAL.printf("[%lu] [WCS] Updated credentials for: %s\n", millis(), ssid.c_str());
    return saveToFile();
  }

  if (credentials.size() >= MAX_NETWORKS) {
    INX_SERIAL.printf("[%lu] [WCS] Cannot add more networks, limit of %zu reached\n", millis(), MAX_NETWORKS);
    return false;
  }

  credentials.push_back({ssid, password});
  INX_SERIAL.printf("[%lu] [WCS] Added credentials for: %s\n", millis(), ssid.c_str());
  return saveToFile();
}
...
bool WifiCredentialStore::removeCredential(const std::string& ssid) {
  ...
  if (cred != credentials.end()) {
    ...
    INX_SERIAL.printf("[%lu] [WCS] Removed credentials for: %s\n", millis(), ssid.c_str());
    return saveToFile();
  }
  return false;
}
...
void WifiCredentialStore::clearAll() {
  credentials.clear();
  saveToFile();
  INX_SERIAL.printf("[%lu] [WCS] Cleared all WiFi credentials\n", millis());
}
