...
bool seekFileInWatchdogFriendlyChunks(FsFile& file, const uint64_t target) {
  ...
  if (target < current) {
    // SdFat follows a FAT chain from the first cluster when seeking backwards. Reset to the start
    // explicitly, then advance in bounded pieces so one large fragmented-file seek cannot starve
    // the ESP32 interrupt watchdog.
    if (!file.seekSet(0)) {
      return false;
    }
    current = 0;
...
bool StarDictLookup::lookupViaCheckpoints(const std::string& candidate, const bool folded, uint64_t& outDictOffset,
  ...
  // Binary search checkpoints_ for the last checkpoint whose word is <= candidate. The selected
  // comparison order was verified while building the index, so this never uses a binary search on
  // a dictionary with an incompatible .idx ordering.
...
bool StarDictLookup::lookup(const std::string& queryWord, std::string& outDefinition, bool* outTruncated) {
  ...
  if (!found) {
    ...
    // A malformed or genuinely unsorted index still needs a sequential fallback. Scan it once for
    // every base-form candidate rather than rescanning the entire dictionary for each one.
