...
#include "../Activity.h"
#include "activity/page/components/home/HomeWidgetLayout.h"
...
class SleepActivity final : public Activity {
 public:
  ...
  explicit SleepActivity(GfxRenderer& renderer, MappedInputManager& mappedInput)
      : Activity("Sleep", renderer, mappedInput), widgetLayout_(renderer) {}
 ...
 private:
   ...
   *
   * Loads BMP/JPG/JPEG/PNG from /sleep/ or root sleep image (fixed choice in settings, or random).
  ...
  /**
   * @brief Renders a minimal clock sleep screen using the Sticky RTC.
   */
  void renderDateTimeSleepScreen() const;

  /** Renders the configured Sleep theme widgets. */
  void renderWidgetSleepScreen() const;
   ...
   * semi-transparent sleep BMP/PNG on top when configured. Non-EPUB books still use the cover bitmap as the base.
   */
  void renderTransparentSleepScreen() const;

  HomeWidgetLayout widgetLayout_;
};
