...
void DictionaryManagerActivity::startInstallation() {
  ...
  if (!displayTaskHandle_ &&
      xTaskCreatePinnedToCore(&DictionaryManagerActivity::displayTaskTrampoline, "DictDisplayTask",
                              kDisplayTaskStack, this, 1, &displayTaskHandle_, 1) != pdPASS) {
    state_ = State::Failed;
    status_ = "Could not start download.";
    updateDisplay();
    return;
  ...
  if (xTaskCreatePinnedToCore(&DictionaryManagerActivity::installTaskTrampoline, "DictInstallTask",
                              kInstallTaskStack, this, 1, &installTaskHandle_, 0) != pdPASS) {
    installTaskHandle_ = nullptr;
    state_ = State::Failed;
    status_ = "Could not start download.";
    updateRequired_ = true;
