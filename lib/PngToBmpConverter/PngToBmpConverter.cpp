...
#endif
  if (memory && zero) std::memset(memory, 0, bytes);
  return memory;
}

inline void servicePngWatchdog() {
#ifdef ARDUINO
  ...
  switch (filterType) {
    ...
    case PNG_FILTER_SUB:
      for (uint32_t i = bpp; i < ctx.rawRowBytes; i++) {
        ...
        if ((i & 0x1FF) == 0) servicePngWatchdog();
      }
      break;
  ...
  for (uint32_t y = 0; y < height; y++) {
    ...
    if (!needsScaling) {
      ...
      if (USE_8BIT_OUTPUT && !oneBit) {
        ...
        for (int x = 0; x < outWidth; x++) {
          const uint8_t adjustedGray = darkenOneBitPngGray(grayRow[x]);
          const uint8_t bit = atkinson1BitDitherer
                                  ? atkinson1BitDitherer->processPixel(adjustedGray, x)
                                  : quantize1bit(adjustedGray, x, y);
      ...
      while (srcY_fp >= nextOutY_srcStart && currentOutY < outHeight) {
        ...
        if (USE_8BIT_OUTPUT && !oneBit) {
          ...
          for (int x = 0; x < outWidth; x++) {
            const uint8_t gray = (rowCount[x] > 0) ? (rowAccum[x] / rowCount[x]) : 0;
            const uint8_t adjustedGray = darkenOneBitPngGray(gray);
            const uint8_t bit = atkinson1BitDitherer
                                    ? atkinson1BitDitherer->processPixel(adjustedGray, x)
                                    : quantize1bit(adjustedGray, x, currentOutY);
            const int byteIndex = x / 8;
            const int bitOffset = 7 - (x % 8);
            rowBuffer[byteIndex] |= (bit << bitOffset);
          }
...
bool PngToBmpConverter::pngFileToEpubWebStyle2BitBmpStream(FsFile& pngFile, Print& bmpOut) {
  ...
  auto* grayRow = static_cast<uint8_t*>(allocateConversionScratch(static_cast<size_t>(ctx.width)));
  if (!grayRow) {
    free(grayDw);
    packer.freeBuffers();
    pngDecodeReleaseScanlines(ctx);
