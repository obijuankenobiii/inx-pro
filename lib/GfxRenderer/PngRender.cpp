...
constexpr size_t kPngReadBufferSize = 16 * 1024;
constexpr uint32_t kPngServiceTimeBudgetMs = 16;

class PngServiceBudget {
 public:
  void service() {
    if (millis() - lastServiceAt_ < kPngServiceTimeBudgetMs) return;
    esp_task_wdt_reset();
...
uint8_t* allocatePngIoBuffer() {
  if (auto* internal = static_cast<uint8_t*>(heap_caps_malloc(kPngReadBufferSize, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT))) {
    return internal;
  }
  return static_cast<uint8_t*>(heap_caps_malloc(kPngReadBufferSize, MALLOC_CAP_8BIT));
}
...
bool loadIdatInput(PngContext& ctx) {
  ...
  if (!ctx.readBuf) return false;
  size_t toRead = kPngReadBufferSize;
...
bool PngRender::render(FsFile& pngFile, int x, int y, int targetWidth, int targetHeight, bool cropToFill,
  ...
  if (cropToFill) {
    ...
    srcX = std::max(0, static_cast<int>((sourceW - srcW) * anchor));
  ...
  if (capture && mode == ImageRenderMode::TwoBit) {
    ...
    if (needed > 0 && needed <= capture->capacity && capture->values) {
      capture->width = outW;
      capture->height = outH;
