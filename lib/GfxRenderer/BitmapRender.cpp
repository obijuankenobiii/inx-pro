...
int roundedOutsideRadius(const int width, const int height, const bool subtle) {
  return subtle ? std::max(1, roundedRectCornerRadius(width, height) / 3)
                : roundedRectCornerRadius(width, height);
}
...
void maskBitmapCornersOutsideRounded(const GfxRenderer& gfx, const int x, const int y, const int drawnW,
  ...
  const int r = roundedOutsideRadius(drawnW, drawnH, subtle);
  if (r < 1) {
    return;
  }
...
inline void serviceBitmapRender(uint32_t& lastService) {
  if (millis() - lastService < 8u) {
    return;
  }
  esp_task_wdt_reset();
  yield();
...
void BitmapRender::render(const Bitmap& bitmap, const int x, const int y, const int maxWidth, const int maxHeight,
  ...
  float scale = 1.0f;
  bool isScaled = false;
  const float clampedCropAnchorX = std::max(0.0f, std::min(1.0f, cropAnchorX));
  const int cropPixX = static_cast<int>(std::floor(bitmap.getWidth() * cropX * clampedCropAnchorX));
  ...
  auto emitPixel = [&](const int screenX, const int screenY, const uint8_t val) {
    if ((roundedOutside == BitmapRender::RoundedOutside::PaperOutside ||
        ...
        maskW > 0 && maskH > 0) {
      const bool subtleMask = roundedOutside == BitmapRender::RoundedOutside::SubtlePaperOutside;
      const int radius = roundedOutsideRadius(maskW, maskH, subtleMask);
      if (!pixelInRoundedRectFillShape(screenX, screenY, x, y, maskW, maskH, radius)) {
        return;
      }
    }
...
void BitmapRender::transparent(const Bitmap& bitmap, int x, int y, int maxWidth, int maxHeight,
  ...
  if (!outRow || !rowBuf) {
    INX_SERIAL.printf("[%lu] [GFX] !! Failed to allocate bitmap row buffers\n", millis());
    free(outRow);
    free(rowBuf);
    return;
  }
  ...
  if (bmp->rewindToData() != BmpReaderError::Ok) {
    INX_SERIAL.printf("[%lu] [GFX] Failed to rewind bitmap\n", millis());
    free(outRow);
    free(rowBuf);
    return;
  }
  ...
  for (int bmpY = 0; bmpY < tgtH; bmpY++) {
    if (bmp->readNextRow(outRow, rowBuf) != BmpReaderError::Ok) {
      INX_SERIAL.printf("[%lu] [GFX] Failed to read row %d\n", millis(), bmpY);
      break;
    }
