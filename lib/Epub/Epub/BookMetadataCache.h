...
class BookMetadataCache {
 ...
 public:
  ...
  /** Session-discovered image dimensions live beside book.bin so reopening a
   * book never needs to re-inflate image ZIP entries merely for layout. */
  bool loadImageMetadata(std::vector<ImageMetadataEntry>& out) const;
  bool saveImageMetadata(const std::vector<ImageMetadataEntry>& entries) const;
