...
bool TocNavParser::setup() {
  ...
  if (!parser) {
    INX_SERIAL.printf("[%lu] [NAV] Couldn't allocate memory for parser\n", millis());
    return false;
  }
...
size_t TocNavParser::write(const uint8_t* buffer, const size_t size) {
  ...
  while (remainingInBuffer > 0) {
    ...
    if (!buf) {
      INX_SERIAL.printf("[%lu] [NAV] Couldn't allocate memory for buffer\n", millis());
      XML_StopParser(parser, XML_FALSE);
      XML_SetElementHandler(parser, nullptr, nullptr);
      XML_SetCharacterDataHandler(parser, nullptr);
      XML_ParserFree(parser);
    ...
    if (XML_ParseBuffer(parser, static_cast<int>(toRead), remainingSize == toRead) == XML_STATUS_ERROR) {
      INX_SERIAL.printf("[%lu] [NAV] Parse error at line %lu: %s\n", millis(), XML_GetCurrentLineNumber(parser),
...
void XMLCALL TocNavParser::startElement(void* userData, const XML_Char* name, const XML_Char** atts) {
  ...
  if (self->state >= IN_BODY && strcmp(name, "nav") == 0) {
    ...
    if (isTocNav) {
      ...
      INX_SERIAL.printf("[%lu] [NAV] Found nav toc element\n", millis());
      return;
    }
    return;
  }
...
void XMLCALL TocNavParser::endElement(void* userData, const XML_Char* name) {
  ...
  if (strcmp(name, "nav") == 0 && self->state >= IN_NAV_TOC) {
    self->state = IN_BODY;
    INX_SERIAL.printf("[%lu] [NAV] Finished parsing nav toc\n", millis());
    return;
  }
}
