...
bool TocNcxParser::setup() {
  ...
  if (!parser) {
    INX_SERIAL.printf("[%lu] [TOC] Couldn't allocate memory for parser\n", millis());
    return false;
  }
...
size_t TocNcxParser::write(const uint8_t* buffer, const size_t size) {
  ...
  while (remainingInBuffer > 0) {
    ...
    if (!buf) {
      INX_SERIAL.printf("[%lu] [TOC] Couldn't allocate memory for buffer\n", millis());
      XML_StopParser(parser, XML_FALSE);
      XML_SetElementHandler(parser, nullptr, nullptr);
      XML_SetCharacterDataHandler(parser, nullptr);
      XML_ParserFree(parser);
    ...
    if (XML_ParseBuffer(parser, static_cast<int>(toRead), remainingSize == toRead) == XML_STATUS_ERROR) {
      INX_SERIAL.printf("[%lu] [TOC] Parse error at line %lu: %s\n", millis(), XML_GetCurrentLineNumber(parser),
