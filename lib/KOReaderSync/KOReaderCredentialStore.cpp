...
bool KOReaderCredentialStore::saveToFile() const {
  ...
  INX_SERIAL.printf("[%lu] [KRS] Saving username: %s\n", millis(), username.c_str());
  ...
  file.close();
  INX_SERIAL.printf("[%lu] [KRS] Saved KOReader credentials to file\n", millis());
  return true;
}
...
void KOReaderCredentialStore::setCredentials(const std::string& user, const std::string& pass) {
  username = user;
  password = pass;
  INX_SERIAL.printf("[%lu] [KRS] Set credentials for user: %s\n", millis(), user.c_str());
}
...
void KOReaderCredentialStore::clearCredentials() {
  ...
  password.clear();
  saveToFile();
  INX_SERIAL.printf("[%lu] [KRS] Cleared KOReader credentials\n", millis());
}

void KOReaderCredentialStore::setServerUrl(const std::string& url) {
  serverUrl = url;
  INX_SERIAL.printf("[%lu] [KRS] Set server URL: %s\n", millis(), url.empty() ? "(default)" : url.c_str());
}
...
void KOReaderCredentialStore::setMatchMethod(DocumentMatchMethod method) {
  matchMethod = method;
  INX_SERIAL.printf("[%lu] [KRS] Set match method: %s\n", millis(),
