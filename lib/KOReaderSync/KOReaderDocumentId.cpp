...
std::string KOReaderDocumentId::calculateFromFilename(const std::string& filePath) {
  ...
  INX_SERIAL.printf("[%lu] [KODoc] Filename hash: %s (from '%s')\n", millis(), result.c_str(), filename.c_str());
  return result;
}
...
std::string KOReaderDocumentId::calculate(const std::string& filePath) {
  ...
  if (!SdMan.openFileForRead("KODoc", filePath, file)) {
    INX_SERIAL.printf("[%lu] [KODoc] Failed to open file: %s\n", millis(), filePath.c_str());
    return "";
  }

  const size_t fileSize = file.fileSize();
  INX_SERIAL.printf("[%lu] [KODoc] Calculating hash for file: %s (size: %zu)\n", millis(), filePath.c_str(), fileSize);
  ...
  for (int i = -1; i < OFFSET_COUNT - 1; i++) {
    ...
    if (!file.seekSet(offset)) {
      INX_SERIAL.printf("[%lu] [KODoc] Failed to seek to offset %zu\n", millis(), offset);
      continue;
    }
  ...
  INX_SERIAL.printf("[%lu] [KODoc] Hash calculated: %s (from %zu bytes)\n", millis(), result.c_str(), totalBytesRead);
