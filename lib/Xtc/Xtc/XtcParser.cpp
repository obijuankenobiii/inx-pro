...
XtcError XtcParser::open(const char* filepath) {
  ...
  if (m_lastError != XtcError::OK) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to read header: %s\n", millis(), errorToString(m_lastError));
    m_file.close();
    return m_lastError;
  }
  ...
  if (m_header.hasMetadata) {
    ...
    if (m_lastError != XtcError::OK) {
      INX_SERIAL.printf("[%lu] [XTC] Failed to read title: %s\n", millis(), errorToString(m_lastError));
      m_file.close();
      return m_lastError;
    }
    m_lastError = readAuthor();
    if (m_lastError != XtcError::OK) {
      INX_SERIAL.printf("[%lu] [XTC] Failed to read author: %s\n", millis(), errorToString(m_lastError));
      m_file.close();
      return m_lastError;
    }
  }
  ...
  if (m_lastError != XtcError::OK) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to read page table: %s\n", millis(), errorToString(m_lastError));
    m_file.close();
    return m_lastError;
  }
  ...
  if (m_lastError != XtcError::OK) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to read chapters: %s\n", millis(), errorToString(m_lastError));
    m_file.close();
    return m_lastError;
  }

  m_isOpen = true;
  INX_SERIAL.printf("[%lu] [XTC] Opened file: %s (%u pages, %dx%d)\n", millis(), filepath, m_header.pageCount,
...
XtcError XtcParser::readHeader() {
  ...
  if (m_header.magic != XTC_MAGIC && m_header.magic != XTCH_MAGIC) {
    INX_SERIAL.printf("[%lu] [XTC] Invalid magic: 0x%08X (expected 0x%08X or 0x%08X)\n", millis(), m_header.magic,
  ...
  INX_SERIAL.printf("[%lu] [XTC] Header: magic=0x%08X (%s), ver=%u.%u, pages=%u, bitDepth=%u\n", millis(), m_header.magic,
...
XtcError XtcParser::readTitle() {
  ...
  INX_SERIAL.printf("[%lu] [XTC] Title: %s\n", millis(), m_title.c_str());
  return XtcError::OK;
}
...
XtcError XtcParser::readAuthor() {
  ...
  INX_SERIAL.printf("[%lu] [XTC] Author: %s\n", millis(), m_author.c_str());
  return XtcError::OK;
}

XtcError XtcParser::readPageTable() {
  if (m_header.pageTableOffset == 0) {
    INX_SERIAL.printf("[%lu] [XTC] Page table offset is 0, cannot read\n", millis());
    return XtcError::CORRUPTED_HEADER;
  }

  if (!m_file.seek(m_header.pageTableOffset)) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to seek to page table at %llu\n", millis(), m_header.pageTableOffset);
    return XtcError::READ_ERROR;
  }
  ...
  for (uint16_t i = 0; i < m_header.pageCount; i++) {
    ...
    if (bytesRead != sizeof(PageTableEntry)) {
      INX_SERIAL.printf("[%lu] [XTC] Failed to read page table entry %u\n", millis(), i);
      return XtcError::READ_ERROR;
    }
  ...
  INX_SERIAL.printf("[%lu] [XTC] Read %u page table entries\n", millis(), m_header.pageCount);
  return XtcError::OK;
}
...
size_t XtcParser::loadPage(uint32_t pageIndex, uint8_t* buffer, size_t bufferSize) {
  ...
  if (!m_file.seek(page.offset)) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to seek to page %u at offset %lu\n", millis(), pageIndex, page.offset);
  ...
  if (headerRead != sizeof(XtgPageHeader)) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to read page header for page %u\n", millis(), pageIndex);
    m_lastError = XtcError::READ_ERROR;
    return 0;
  }
  ...
  if (pageHeader.magic != expectedMagic) {
    INX_SERIAL.printf("[%lu] [XTC] Invalid page magic for page %u: 0x%08X (expected 0x%08X)\n", millis(), pageIndex,
  ...
  if (bufferSize < bitmapSize) {
    INX_SERIAL.printf("[%lu] [XTC] Buffer too small: need %u, have %u\n", millis(), bitmapSize, bufferSize);
  ...
  if (bytesRead != bitmapSize) {
    INX_SERIAL.printf("[%lu] [XTC] Page read error: expected %u, got %u\n", millis(), bitmapSize, bytesRead);
    m_lastError = XtcError::READ_ERROR;
    return 0;
  }
