...
bool Xtc::load() {
  INX_SERIAL.printf("[%lu] [XTC] Loading XTC: %s\n", millis(), filepath.c_str());

  parser.reset(new xtc::XtcParser());

  xtc::XtcError err = parser->open(filepath.c_str());
  if (err != xtc::XtcError::OK) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to load: %s\n", millis(), xtc::errorToString(err));
    parser.reset();
    return false;
  }

  loaded = true;
  INX_SERIAL.printf("[%lu] [XTC] Loaded XTC: %s (%lu pages)\n", millis(), filepath.c_str(), parser->getPageCount());
  return true;
}

bool Xtc::clearCache() const {
  if (!SdMan.exists(cachePath.c_str())) {
    INX_SERIAL.printf("[%lu] [XTC] Cache does not exist, no action needed\n", millis());
    return true;
  }

  if (!SdMan.removeDir(cachePath.c_str())) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to clear cache\n", millis());
    return false;
  }

  INX_SERIAL.printf("[%lu] [XTC] Cache cleared successfully\n", millis());
  return true;
}
...
bool Xtc::generateCoverBmp() const {
  ...
  if (!loaded || !parser) {
    INX_SERIAL.printf("[%lu] [XTC] Cannot generate cover BMP, file not loaded\n", millis());
    return false;
  }

  if (parser->getPageCount() == 0) {
    INX_SERIAL.printf("[%lu] [XTC] No pages in XTC file\n", millis());
    return false;
  }
  ...
  if (!parser->getPageInfo(0, pageInfo)) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to get first page info\n", millis());
    return false;
  }
  ...
  if (!pageBuffer) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to allocate page buffer (%lu bytes)\n", millis(), bitmapSize);
    return false;
  }

  size_t bytesRead = const_cast<xtc::XtcParser*>(parser.get())->loadPage(0, pageBuffer, bitmapSize);
  if (bytesRead == 0) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to load cover page\n", millis());
    free(pageBuffer);
    return false;
  }
  ...
  if (!SdMan.openFileForWrite("XTC", getCoverBmpPath(), coverBmp)) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to create cover BMP file\n", millis());
    free(pageBuffer);
    return false;
  }
  ...
  INX_SERIAL.printf("[%lu] [XTC] Generated cover BMP: %s\n", millis(), getCoverBmpPath().c_str());
  return true;
}
...
bool Xtc::generateThumbBmp() const {
  ...
  if (!loaded || !parser) {
    INX_SERIAL.printf("[%lu] [XTC] Cannot generate thumb BMP, file not loaded\n", millis());
    return false;
  }
  ...
  if (scale >= 1.0f) {
    if (generateCoverBmp()) {
      ...
      INX_SERIAL.printf("[%lu] [XTC] Copied cover to thumb (no scaling needed)\n", millis());
  ...
  INX_SERIAL.printf("[%lu] [XTC] Generating thumb BMP: %dx%d -> %dx%d (scale: %.3f)\n", millis(), pageInfo.width,
  ...
  size_t bytesRead = const_cast<xtc::XtcParser*>(parser.get())->loadPage(0, pageBuffer, bitmapSize);
  if (bytesRead == 0) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to load cover page for thumb\n", millis());
    free(pageBuffer);
    return false;
  }
  ...
  if (!SdMan.openFileForWrite("XTC", getThumbBmpPath(), thumbBmp)) {
    INX_SERIAL.printf("[%lu] [XTC] Failed to create thumb BMP file\n", millis());
    free(pageBuffer);
    return false;
  }
  ...
  INX_SERIAL.printf("[%lu] [XTC] Generated thumb BMP (%dx%d): %s\n", millis(), thumbWidth, thumbHeight,
