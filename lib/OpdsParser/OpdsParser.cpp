...
OpdsParser::OpdsParser() {
  ...
  if (!parser) {
    errorOccured = true;
    INX_SERIAL.printf("[%lu] [OPDS] Couldn't allocate memory for parser\n", millis());
  }
}
...
size_t OpdsParser::write(const uint8_t* xmlData, const size_t length) {
  ...
  while (remaining > 0) {
    ...
    if (!buf) {
      errorOccured = true;
      INX_SERIAL.printf("[%lu] [OPDS] Couldn't allocate memory for buffer\n", millis());
      XML_ParserFree(parser);
      parser = nullptr;
      return length;
    }
    ...
    if (XML_ParseBuffer(parser, static_cast<int>(toRead), 0) == XML_STATUS_ERROR) {
      errorOccured = true;
      INX_SERIAL.printf("[%lu] [OPDS] Parse error at line %lu: %s\n", millis(), XML_GetCurrentLineNumber(parser),
